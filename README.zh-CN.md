...
**每条都有中文**：一句话简介 + 点开看 300 字详细介绍，不用先读懂英文原文。
...
装完就是一个独立图标，**打开即用**；已在 iPhone Safari 与 Android Chrome 上测试过
「添加到主屏幕」，装好后离线也能打开（数据缓存，联网自动刷新）。
...
> ⚠️ **这条 cron 是否有效，取决于服务器所在地域。**
> 中国大陆节点通常访问不到 `raw.githubusercontent.com`，命令会**静默失败**——
> 站点看着在跑，数据却停在几天前。若部署在大陆节点，请改用你自己的数据管道
> 或国内可达的镜像源；中国香港／海外节点可直接用上面这条。
...
本工具展示的内容来自公开渠道（GitHub 公开信息、阮一峰《科技爱好者周刊》），
**不含任何个人信息**。

- 本应用**不收集、不上传任何用户数据**，无账号、无埋点、无第三方统计。
- 全部逻辑在你自己的浏览器里跑，`data.json` 是静态文件，不需要请求任何后端。
- 若你自行部署，请同样**不要**往公开仓库或 Issue 里粘贴任何个人信息。

## 来源与许可

- **代码**：MIT License，见 [LICENSE](LICENSE)。
- **数据**：项目名称、描述、星数等来自 GitHub 公开 API；部分推荐条目来自
  阮一峰《科技爱好者周刊》**公开页面**。
- **第三方内容**：各项目信息与周刊内容**版权归各原作者所有**；本仓库**仅收录
  标题、原文链接与自动生成的中文简介，不转载第三方原文**。本仓库的 MIT 许可
  **不覆盖**上述第三方内容。

## 免责

- 本仓库的中文简介与详细介绍**由自动流程生成**，仅供快速了解，**请以项目官方
  README 与说明为准**。
- 本工具**不提供**任何项目的安装、编译或使用指导；收录不代表推荐或安全背书，
  采用前请自行评估。
- 当前为 **v0.1 初版**：分类规则与中文翻译仍在完善，可能出现归类不当或译述偏差，
  欢迎提交 Issue 纠错。
